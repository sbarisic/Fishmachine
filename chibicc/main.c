...
static void add_default_include_paths(char* argv0) {
	...
	strarray_push(&include_paths, format("%s/include", dirname(_strdup(argv0))));
...
static void parse_args(int argc, char** argv) {
	...
	memset(&idirafter, 0, sizeof(StringArray));
...
static char* find_file(char* pattern) {
	...
	if (hFind != INVALID_HANDLE_VALUE) {
		...
		char* fullPath = format("%s%s", dir, findData.cFileName);
		if (path)
			free(path);
		path = _strdup(fullPath);
		free(fullPath);

		// Find all matches and keep the last one (to match original behavior)
		while (FindNextFileA(hFind, &findData)) {
			fullPath = format("%s%s", dir, findData.cFileName);
			if (path)
				free(path);
			path = _strdup(fullPath);
			free(fullPath);
...
static void run_linker(StringArray* inputs, char* output) {
	StringArray arr;
	memset(&arr, 0, sizeof(StringArray));
...
int main(int argc, char** argv) {
	...
	StringArray ld_args;
	memset(&ld_args, 0, sizeof(StringArray));

...
namespace CodeGeneration
{
    public class CGenState
    {
        ...
        private void PUSHL(Reg src) => this.os.WriteLine($"    PUSH_REG {RegToString(src)}");

        private void PUSHL(Int32 imm) => this.os.WriteLine($"    PUSH_LONG ${imm}");
        ...
        /// </summary>
        public void MOVL(string instr, String src, String dst) => this.os.WriteLine($"    {instr} {src}, {dst}");

        public void MOVL(String src, Reg dst) => MOVL("MOVE_LONG_REG", src, RegToString(dst));

        //public void MOVL(Int32 imm, String dst) => MOVL("MOVE_LONG_LONG", $"${imm}", dst);

        public void MOVL(Int32 imm, Reg dst) => MOVL("MOVE_LONG_REG", $"${imm}", RegToString(dst));

        public void MOVL(Reg src, Reg dst) => MOVL("MOVE_REG_REG", RegToString(src), RegToString(dst));

        public void MOVL(Reg src, Int32 offset, Reg dst) => MOVL("MOVE_REG_OFFSET_REG", RegToString(src), $"{offset}, {RegToString(dst)}");

        public void MOVL(Int32 offset, Reg src, Reg dst) => MOVL("MOVE_OFFSET_REG_REG", $"{offset}, {RegToString(src)}", RegToString(dst));
        ...
        // ===
        // 
        public void LEA(string instr, String addr, String dst) => this.os.WriteLine($"    {instr} {addr}, {dst}");

        public void LEA(String addr, Reg dst) => LEA("LEA_ADDR_REG", addr, RegToString(dst));

        public void LEA(Int32 offset, Reg src, Reg dst) => LEA("LEA_OFFSET_REG_REG", $"{offset}, {RegToString(src)}", RegToString(dst));
        ...
        public void CGenExpandStackTo(Int32 size, String comment = "")
        {
            ...
            {
                SUBL(size - this.StackSize, Reg.ESP, comment);
                this.StackSize = size;
            }
        }
        ...
        {
            nbytes = CodeGenUtils.RoundUp(this.StackSize + nbytes, align) - this.StackSize;
        ...
        /// SUBL: subtract long
        /// </summary>
        public void SUBL(string instr, String er, String ee, String comment = "")
        {
            this.os.Write($"    {instr} {er}, {ee}");
            if (comment == "")
            {
        ...
        public void SUBL(Int32 er, Reg ee, String comment = "") => SUBL("SUB_LONG_REG", $"${er}", RegToString(ee), comment);

        public void SUBL(Reg er, Reg ee, String comment = "") => SUBL("SUB_REG_REG", RegToString(er), RegToString(ee), comment);
